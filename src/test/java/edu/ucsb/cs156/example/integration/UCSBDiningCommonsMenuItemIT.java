...
public class UCSBDiningCommonsMenuItemIT {
        ...
        @Test
        public void an_admin_user_can_post_a_new_dcmi() throws Exception {
            ...
            MvcResult response = mockMvc.perform(
                post("/api/ucsbdiningcommonsmenuitem/post?name=Cheeseburger&diningCommonsCode=DLG&station=Grill")
                .with(csrf()))
                .andExpect(status().isOk()).andReturn();
            
            String expectedJson = mapper.writeValueAsString(ucsbDCMI1);
            String responseJson = response.getResponse().getContentAsString();
            assertEquals(expectedJson, responseJson);
        }
}